app.controller("mainController", function($scope, $http){
	...
	$scope.apiKey = "21ace55046b1842f9fa91ded6b63440b";
	$scope.results = [];
	$scope.filterText = null;
	$scope.genreFilter = null;
	$scope.availableGenres = [];
	$scope.orderFields = ["Air Date", "Rating"];
	$scope.orderField = "Air Date"; // Default Air Date
  ...
  $scope.init = function() {
    ...
    $http.jsonp('http://api.trakt.tv/calendar/premieres.json/' + $scope.apiKey + '/' + apiDate + '/' + 30 + '/?callback=JSON_CALLBACK').success(function(data) {
      ...
      angular.forEach(data, function(value, index){
				...
				var date = value.date;
				// Loop through episodes and add it to the results array
				angular.forEach(value.episodes, function(tvshow, index){
					// create a date string from the timestamp so we can filter on it based on user input
					tvshow.date = date;
